...
namespace GearMenu
{
    ...
    public static class Buttons
    {
        ...
        public static void SetText(GameObject go, string label)
        {
            ...
            t.onHoverOver.Clear();
            EventDelegate.Add(t.onHoverOver, () => { InternalSetText(label, label != null); });
            var b = go.GetComponent<UIButton>();

            // 既にホバー中なら説明を変更する
            if (b.state == UIButtonColor.State.Hover)
            {
                InternalSetText(label, label != null);
                //SysShortcut.VisibleExplanation(label, label != null);
            }
        }

        private static void InternalSetText(string text, bool isVisible)
        {
            if (labelExplanationField == null)
            {
                var t = typeof(SystemShortcut);
                labelExplanationField = t.GetField("m_labelExplanation", BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.CreateInstance);
                spriteExplanationField = t.GetField("m_spriteExplanation", BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.CreateInstance);
            }
            var label = labelExplanationField.GetValue(SysShortcut) as UILabel;
            var sprite = spriteExplanationField.GetValue(SysShortcut) as UISprite;
            ...
            if (isVisible)
            {
                ...
                var component = sprite.GetComponent<UISprite>();
                component.width = label.width + 15;
            }
        ...
        private static FieldInfo labelExplanationField;
        private static FieldInfo spriteExplanationField;
